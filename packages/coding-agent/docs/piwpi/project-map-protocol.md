...
2. 累计未整理文件数 ≥ **5** 或累计行数 ≥ **1000**（HarnessOptions 可注入）→ 触发**批量整理**。
   文件数阈值**每轮（agent_settled）递减 1，下限 1**，触发后重置为初始值——永远达不到阈值的小量
   pending 会在有限轮数内攒成一**批**触发（一次调用），而不是每轮都整理：
   - 收集全部 pending 插件，**整批一次**调用记忆 Agent（经记忆队列链路，不阻塞主流程）；
   - 输入域严格限定为三段（记忆 Agent **无外部工具能力**）：
     1. 各文件挂载内容（确定性渲染，`render`）；
     2. 主 Agent 最近对话尾部（最近 6 条消息的去重摘要——toolResult 只保留标记行，
        与挂载内容**永不重叠**）；
     3. Project Map 已有条目精简列表（`renderBrief`：路径 — 角色，每项一行）。
   - 模型输出 `entries`（pluginId → role/responsibilities）→ 逐个 `projectMap.update()`
     （整体替换，新整理结果覆盖旧内容）→ 整批一次落盘 + 会话 custom entries；
     模型漏掉的 pluginId 保留 pending，下轮再试；
